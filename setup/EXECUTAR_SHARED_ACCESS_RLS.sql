...
-- usuários convidados vejam os dados dos usuários que compartilharam.
--
-- IMPORTANTE: Execute este SQL logado como administrador no Supabase.
-- ============================================================
...
DROP POLICY IF EXISTS "Users can view own or shared transactions" ON public.transactions;
...
DROP POLICY IF EXISTS "Users can view own or shared budgets" ON public.budgets;
...
-- ================================================
-- RECEIVABLES - Update policies for shared access
-- ================================================

DROP POLICY IF EXISTS "Users can view their own receivables" ON public.receivables;
DROP POLICY IF EXISTS "Users can view own or shared receivables" ON public.receivables;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'receivables') THEN
    EXECUTE 'CREATE POLICY "Users can view own or shared receivables" ON public.receivables
      FOR SELECT USING (
        user_id = auth.uid() 
        OR user_id IN (
          SELECT owner_id FROM public.account_members 
    ...
    EXECUTE 'CREATE POLICY "Users can insert their own receivables" ON public.receivables
      FOR INSERT WITH CHECK (user_id = auth.uid())';
    
    EXECUTE 'CREATE POLICY "Users can update their own receivables" ON public.receivables
      FOR UPDATE USING (user_id = auth.uid())';
    
    EXECUTE 'CREATE POLICY "Users can delete their own receivables" ON public.receivables
      FOR DELETE USING (user_id = auth.uid())';
  END IF;
END $$;

-- ================================================
-- ASSET VALUATIONS - Update policies for shared access
-- ================================================

DROP POLICY IF EXISTS "Users can view their own asset_valuations" ON public.asset_valuations;
DROP POLICY IF EXISTS "Users can view own or shared asset_valuations" ON public.asset_valuations;
DROP POLICY IF EXISTS "Users can insert their own asset_valuations" ON public.asset_valuations;
DROP POLICY IF EXISTS "Users can update their own asset_valuations" ON public.asset_valuations;
DROP POLICY IF EXISTS "Users can delete their own asset_valuations" ON public.asset_valuations;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'asset_valuations') THEN
    EXECUTE 'CREATE POLICY "Users can view own or shared asset_valuations" ON public.asset_valuations
      FOR SELECT USING (
        user_id = auth.uid() 
        OR user_id IN (
          SELECT owner_id FROM public.account_members 
    ...
    EXECUTE 'CREATE POLICY "Users can insert their own asset_valuations" ON public.asset_valuations
      FOR INSERT WITH CHECK (user_id = auth.uid())';
    
    EXECUTE 'CREATE POLICY "Users can update their own asset_valuations" ON public.asset_valuations
      FOR UPDATE USING (user_id = auth.uid())';
    
    EXECUTE 'CREATE POLICY "Users can delete their own asset_valuations" ON public.asset_valuations
      FOR DELETE USING (user_id = auth.uid())';
  END IF;
END $$;
...
-- FIM DA MIGRATION
-- ============================================================
-- Após executar este SQL, os usuários convidados poderão ver
-- os dados dos usuários que compartilharam suas contas.
-- ============================================================
