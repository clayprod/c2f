...
export async function PATCH(
  ...
  try {
    ...
    if (hasCustomPlan && validated.plan_entries && validated.plan_entries.length > 0) {
      const totalAmount = validated.total_amount_cents ?? currentReceivable.total_amount_cents;
      const receivedAmount = validated.received_amount_cents ?? currentReceivable.received_amount_cents ?? 0;
      ...
      if (planTotal < remainingAmount) {
        ...
        console.warn(`Receivable ${id}: Plan total (${planTotal}) is less than remaining amount (${remainingAmount})`);
      }
    }
    ...
    const contributionFrequencyChanged = validated.contribution_frequency !== undefined && validated.contribution_frequency !== currentReceivable.contribution_frequency;
